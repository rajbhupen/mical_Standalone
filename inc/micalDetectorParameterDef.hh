...
double GetAlTileBase() {return AlTileBase;}
  double GetGapBtwTiles() {return GapBtwTiles;}
  double GetTileWidth() {return TileWidth;}
    double GetNoofEPSinTile() {return NoofEPSinTile;}
...
int   NoofTilesonSideWall;
  ...
  //cmv
  // to be used in micalCal1sd.cc to retrieve position of physical vol.
  //  double ScintLayGPos[4][4][3]={{{0}}};
 double ScintLayGPos[7][4][3]={{{0}}};
  double fiberDia;//1.4mm
  double fiberXpos;//12.5mm
 int NoScntStrpTop; //= 88;
int NoScntStrpSide; //= 40;
  int NoScntStrpSideSmallay; //8
float ThkofScntStrpTop; //=1.8*cm;
  double  SidePlaneHalfLength;
  double SideSmallPlaneHalfLength;
   double  TopPlaneHalfLength;

double ScntLayShifTop;    //=(2*partopscint[0]+AirGapScintTop)/4; // 1/4 shift
double ScntLayShifSide;   //= (2*partopscint[0]+AirGapScintTop)/3; // 1/3 shift
...
private : 
ParameterMessenger* detectorConfig;
  int numino;
  double nFeThickness;
  float nAirGap ;
  ...
  double INOroomPos[3];
  double RPCLayerPosZ[11];
  double IRONLayerPosZ[12];
 ...
 // float AirGapScintTop;
  ...
  //float partopscint[3]; moved to public
  ...
  double LayerZdim[11]; //added wo more layer
  double IronLayerZdim[11];

  double LayerPosInStack[24]; //added four (2 x 2) more chamber
