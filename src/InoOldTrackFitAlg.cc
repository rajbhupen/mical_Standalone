//2022,RSA: Only *_new functions are used for trackfitting.
//2022,RSA CMVD & Roof extrapolation is included.
...
InoOldTrackFitAlg::InoOldTrackFitAlg()
{  // cout<<"  InoOldTrackFitAlg::InoOldTrackFitAlg() " <<endl;
 ...
 inoTrackCand_pointer = InoTrackCand_Manager::APointer;
  ...
  paramess = ParameterMessenger::AnPointer; //to use gdml option



  gdmlOption = paramess->GetgdmlOption();
  ...
  ShiftInZ =paradef->GetShiftInZ(0);//RSA
  ...
  for (int ijk=0; ijk<nLayer; ijk++) {
   ...
   // cout<<"ZPosLayer["<< ijk<<"]:  "<<ZPosLayer[ijk]<<endl;
    //RSA
    ZPosLayer[ijk] = (1/m)*(paradef->GetRPCLayerPosZ(ijk)+ShiftInZ+paradef->GetINOroomPos(2)+ paradef->GetStackPosInRoom(2));
    cout<<"ZPosLayer["<< ijk<<"]:  "<<ZPosLayer[ijk]<<endl;
    
  }
  //ZPosLayer[11]=   paradef->GetINOroomPos(2)+paradef->GetStackPosInRoom(2)+paradef->GetParmagnet(2);
  ZPosLayer[10]=   ZPosLayer[9]+0.101;
  ZPosLayer[11]=   (1/m)*(paradef->GetParworld(2)-1);//Building height
 cout<<"ZPosLayer[11]: "<< ZPosLayer[11]<<endl;
  UseGeoSwimmer = 0;
  if(pAnalysis->isXtermOut==1){
    cout <<"\n InoOldTrackFitAlg::InoOldTrackFitAlg() "<<StripXWidth<<" "<<StripYWidth<<" "<<LayerThickness<<endl;
  }
}
...
void InoOldTrackFitAlg::RunAlg( ) {
  cout<<"void InoOldTrackFitAlg::RunAlg( ) {"<<endl;
  ...
  if(debug_fit)
    cout <<"----------------------InoOldTrackFitAlg::RunAlg( ) "<<ptrackCollection->InoTrack_list.size()<<"----------------------"<<endl;
  if (ptrackCollection) {
	...
	if(fTrackCand->ClustsInTrack.size()<4) break; //RSA
	  ...
	  for(unsigned int kpc=0; kpc<fTrackCand->ClustsInTrack.size(); kpc++) {
	    cout<<"clust "<<kpc<<" = ("<<fTrackCand->ClustsInTrack[kpc]->GetXPos()<<","<<fTrackCand->ClustsInTrack[kpc]->GetYPos()<<","<<fTrackCand->ClustsInTrack[kpc]->GetZPos()<<","<<","<<fTrackCand->ClustsInTrack[kpc]->GetZPlane()<<          ");"<<endl;
	  }
	}
	...
	cout <<" fittrer "<< inoTrackCand_pointer <<" " <<inoTrackCand_pointer->InoTrackCand_list.size()<<endl;
...
void InoOldTrackFitAlg::InitialFramework_new( ) {

  if(debug_fit) 
    cout<<"----------------------InoOldTrackFitAlg::InitialFramework_new()----------------------" <<endl;
  ...
  //  Store all track clusters found,
    cout<<"InoOldTrackFitAlg::InitialFramework total hitlistx "<<pinoclust->InoCluster_list.size()<<" Hists in track "<<fFinderTrack->ClustsInTrack.size()<<endl;
   ...
   cout <<"Exiting InitialFramework "<<MinPlane<<" "<<MaxPlane<<endl;
} 


void InoOldTrackFitAlg::ShowerStrips() {
  //a  cout <<"InoOldTrackFitAlg : ShowerStrips, Look for large vertex shower" << endl;
...
void InoOldTrackFitAlg::RunTheFitter_new( ) {
  
  if(debug_fit) 
    cout<< "---------------------- InoOldTrackFitAlg::RunTheFitter_new( ) ----------------------" <<endl;
  //MultiSimAnalysis *pAnalysis = MultiSimAnalysis::AnPointer;
  //cout <<"InoOldTrackFitAlg : RunTheFitter_new, Call methods in the appropriate order" << endl;
  GetInitialCovarianceMatrix(true);
  //cout <<"InoOldTrackFitAlg : RunTheFitter_new GetInitialCovarianceMatrix"<<endl;
  const bool GoForward=true;
  const bool GoBackward=false;
  double xvtx_parameter[6] = {0.0};//cmvd
  	double xvtx_para4roof[6] = {0.0}; //RSA parameters on roof
	...
	ExtraPolData[i].clear();
	  ...
	  if (nint>0) {
	    ...
	    GetFitData_new(MinPlane,MaxPlane);
          ...
          if (MinPlane > MaxPlane) { cout<<" PassTrack 3" << MinPlane<< "<"<< MaxPlane<<" ievt "<<pAnalysis->ievt<<endl;PassTrack=false; break;} //ievt2 RSA
      ...
      else {cout<<" PassTrack 3.1 " <<" ievt "<<pAnalysis->ievt<<endl; PassTrack=false;}//ievt2 RSA
    }
  ...
  // Control iterations over a track for which ZDecreasesWithTime
  else
    {
      ...
      LastIteration=false;
      cout<<"Called GoBackwards_new(false); first time:  "<<endl;
 ...
 cout<<"Called GoForwards_new(false); first time:  "<<endl;
	  ...
	  if (nint>0) {
	    ...
	    GetFitData_new(MinPlane,MaxPlane);
          ...
          if (MinPlane > MaxPlane) { cout<<" PassTrack 3.2 " << MinPlane<< "<"<< MaxPlane<<" ievt "<<pAnalysis->ievt<<endl; PassTrack=false; break;} //rsa
          SaveData=false; 
	  ...
	  if ((nint == niteration || ndifchi==0 )&&nint>1) {
	    for(int i=0; i<6; ++i) {StateVector[i]=x_k_minus[i];}
	    	for (int i=0; i<6; i++) { cout <<i<<" "<<StateVector[i]<<" "<<endl;}
	    // double loc_zend = 0.0; //RSA
	...
	//.......................................CMVD Extrapolation..............................
	if(LastIteration){
	  cout<<"Extrapolation to CMVD in InoTrackFit Alg: "<< ZPosLayer[10]<<" "<<ZPosLayer[11]<<endl;
	
			double tmpmom =(abs(x_k_minus[4])>1.e-12) ? 1/x_k_minus[4] : 0;
		       	cout <<"MinXPlane " <<MinPlane<<" "<<MaxPlane<<" "<<x_k_minus[0]<<" "<<x_k_minus[1]<<" "<<x_k_minus[2]<<" "<<x_k_minus[3]<<" "<<x_k_minus[4]<<" "<<" "<<tmpmom<<endl;

			double x_k_minus_copy[5] = {0.0};

			x_k_minus_copy[0] = x_k_minus[0];
			x_k_minus_copy[1] = x_k_minus[1];
			x_k_minus_copy[2] = x_k_minus[2];
			x_k_minus_copy[3] = x_k_minus[3];
			x_k_minus_copy[4] = x_k_minus[4];
			int MaxPlane_copy = MaxPlane;
			double ds=0;
			double drange=0;
			...
			int NPlane=10;//cmvd
			 ...
			 ds=0;
			 drange=0;
			double xvtx_para[6] = {0.0};

     
			//		PredictedStateCov(x_k_minus_copy, MaxPlane, NPlane, GoBackward, xvtx_para, 0, &ds, &drange);//cmvd

			bool GetPrediction=Swim_new(x_k_minus_copy, xvtx_para, MaxPlane_copy, NPlane, true, &ds, &drange);
			tmpmom =(abs(xvtx_para[4])>1.e-12) ? 1/xvtx_para[4] : 0;
			cout <<"extrapol "<<NPlane <<" "<<xvtx_para[0]<<" "<<xvtx_para[1]<<" "<<xvtx_para[2]<<" "<<xvtx_para[3]<<" "<<xvtx_para[4]<<" "<<"Z: "<<xvtx_para[5]<<" "<<tmpmom<<" "<<ds<<" "<<drange<<" "<<ZPosLayer[NPlane]<<" "<<zexppos<<endl;
			xvtx_parameter[0] = (xvtx_para[4]!=0) ? 1./xvtx_para[4] : -100;
			double dsdzvx = sqrt(1.0 + xvtx_para[2]*xvtx_para[2] + xvtx_para[3]*xvtx_para[3]);
			xvtx_parameter[1] = acos(-1./dsdzvx);
			xvtx_parameter[2] = atan2(-xvtx_para[3], -xvtx_para[2]);
			xvtx_parameter[3] = xvtx_para[0];
			xvtx_parameter[4] = xvtx_para[1];
			xvtx_parameter[5] =1.e+3*zexppos;// ZPosLayer[NPlane]; //to be saved in mm since in CMVD reco algorithm it is used in mm (Can be changed)
			   
			}//gdmloption
	        
	///////////////////////////////
			cout<<"Prediction to Top of Buiding for momentum Measurements wrt Gen. value "<<endl;
			//Prediction to Top of Buiding for momentum Measurements wrt Gen. value
			int BuildPlane =11;
		        ds=0;
			drange=0;
			double xvtx_para_roof[6] = {0.0};
			 G4double G_Point[3]={0};
		
			bool GetRoofPrediction=Swim_new(x_k_minus_copy, xvtx_para_roof, MaxPlane_copy, BuildPlane, true, &ds, &drange);
			
			cout <<"extrapol to roof "<<BuildPlane <<" "<<xvtx_para_roof[0]<<" "<<xvtx_para_roof[1]<<" "<<xvtx_para_roof[2]<<" "<<xvtx_para_roof[3]<<" "<<xvtx_para_roof[4]<<" "<<tmpmom<<" "<<ds<<" "<<drange<<" "<<ZPosLayer[BuildPlane]<<" "<<xvtx_para_roof[5]<<endl;	


			xvtx_para4roof[0] = (xvtx_para_roof[4]!=0) ? 1./xvtx_para_roof[4] : -100;
			
			double dsdzvxrf = sqrt(1.0 + xvtx_para_roof[2]*xvtx_para_roof[2] + xvtx_para_roof[3]*xvtx_para_roof[3]);
			xvtx_para4roof[1] = acos(-1./dsdzvxrf);
			xvtx_para4roof[2] = atan2(-xvtx_para_roof[3], -xvtx_para_roof[2]);
			xvtx_para4roof[3] = xvtx_para_roof[0];
			xvtx_para4roof[4] = xvtx_para_roof[1];

	        
			
			xvtx_para4roof[5] =zexppos; //in m

	cout<<"xvtx_para4roof[5]"<<xvtx_para4roof[5]<<endl;
			//If extrapolation goes outside buiding avoid adding edep to momentum.And just do extrapolation using LinePlane Intersection.

	cout<<""<<xvtx_para4roof[5]<<" "<<ZPosLayer[11]<< " " <<ZPosLayer[11] <<endl;
			if(abs(xvtx_para4roof[5]-ZPosLayer[11])>1.e-04 ){
			  //			if(xvtx_para4roof[5] <ZPosLayer[buildplane]){
			  cout<<".........LinePlane Intersection to Roof........."<<endl;
			  G4double Plane[6]={0,0,ZPosLayer[11],0,0,1};
			  for(int ij=0;ij<6;ij++){cout<<Plane[ij]<<" ";}
			  cout<<endl;
			  double PI = acos(-1.0);
   
			  G4ThreeVector dirVector(0,0,1);  //     
			  dirVector.setTheta(PI-xvtx_para4roof[1]);
			  dirVector.setPhi(xvtx_para4roof[2]+PI);
			  G4double Line[6]={xvtx_para4roof[3],xvtx_para4roof[4],xvtx_para4roof[5],dirVector.x(),dirVector.y(),dirVector.z()};
			  // Just do line plane intersection to get  [0] & [1]
			  bool pl2 = LinePlaneInt (Line, Plane, G_Point);

  
			  xvtx_para4roof[3] = G_Point[0];
			  xvtx_para4roof[4] = G_Point[1];
			  xvtx_para4roof[5] =G_Point[2];
      ...
      else {cout<<" PassTrack 3.3 " <<" ievt "<<pAnalysis->ievt<< endl;PassTrack=false;}//rsa
    }
  bool fcpc_col = false;
  FCorPC = 0;
  if (LastIteration && fcpc_col) {
  ...
  if(x_k[4]!=0. && PassTrack==true) {
	...
	//        cth.SetPass(1);
        cout <<"x_k:: "<<x_k[0]<<" "<<x_k[1]<<" "<<x_k[2]<<" "<<x_k[3]<<" "<<x_k[4]<<endl;
	
	// cout<<"Setting track prop1"<<endl;
        SetTrackProperties(x_k,xvtx_parameter,xvtx_para4roof); //cth);//2:for cmvd

      } else { // else of if(NumInUView>1 && NumInVView>1) {
	cout<<" PassTrack 4" << " ievt "<<pAnalysis->ievt<<endl; //asm: PassTrack 4 was noted frequently.//rsa
	PassTrack=false;
      } // if(NumInUView>1 && NumInVView>1) {
    } else { // else of if(FinalClusterFound==true) {
      // Otherwise fail the track at this final stage
      cout<<" PassTrack 6" <<" ievt "<<pAnalysis->ievt<< endl;PassTrack=false;
    } // if(FinalClusterFound==true) {//rsa
  }
...
void InoOldTrackFitAlg::RemoveTrkHitsInShw() {
  // If the 'clean' section of track is large enough, remove the track finding
  // data for planes before the ShowerEntryPlane
  if(debug_fit)
    cout <<"----------------------InoOldTrackFitAlg : RemoveTrkHitsInShw, Discard track finding data in shower ----------------------" << endl;
  ...
  // Otherwise note that we should not run the ShowerSwim method
    ...
    cout <<"InoOldTrackFitAlg : RemoveTrkHitsInShw, not enough hits after removal. Must use all finder data." << endl;
    SwimThroughShower=false;
  }
...
void InoOldTrackFitAlg::ShowerSwim() {
  ...
  if(debug_fit){ 
    cout<<" =========================================================="<<endl;
    cout <<"InoOldTrackFitAlg : ShowerSwim, improved track finding in shower" << endl;
    cout<<" =========================================================="<<endl;
  }
  // Initialisations
  int Plane; int NewPlane;
  ...
  while(PlanesSinceLastHit<4 && NewPlane>0 && NewPlane<=(int)nLayer-5){ //145) { //GMA Put those number from database
	...
	//	  cout <<"InoOldTrackFitAlg.Showerswim : WARNING : PlaneView for hits is not matching with 0/1/2"<<endl;

	double tmp_ds, tmp_drange;
        bool CombiPropagatorOk=GetCombiPropagator_new(Plane,NewPlane,GoForward);
...
void InoOldTrackFitAlg::GetFitData_new(int& Plane1, int& Plane2) {
  // Loop over the initial track strip data and create the final data for fitting
  // if(debug_fit)
  if(TrkFitterDebug>10) {
    cout <<"----------------------InoOldTrackFitAlg : GetFitData_new---------------------- 1,2 "<<Plane1<<" "<<Plane2<<endl ;
  }
  ...
  for (unsigned ijk=0; ijk<doubleLa; ijk++) {
	...
	if(FilteredData[ijk-misLayer].size()){
	  ...
	  ctheta = 0.101/pow(pow(x1-x2,2)+pow(y1-y2,2)+ pow(0.101,2),0.5);
...
void InoOldTrackFitAlg::FillGapsInTrack() {
  ...
  if(TrkFitterDebug>100) {
    ...
    cout <<"----------------------InoOldTrackFitAlg :: FillGapsInTrack----------------------" << endl;
  }
...
bool InoOldTrackFitAlg::GetCombiPropagator(const int Plane, const int NewPlane, const bool GoForward, double* ddS, double* ddRange) {
  // Combination propagator, essentially same as SR propagator, but
  // generation of matrix reduces calls to swimmer by 80%

  if (debug_new) 
    cout <<" ----------------------InoOldTrackFitAlg : GetCombiPropagator(const int Plane, const int NewPlane, const bool GoForward, double* ddS, double* ddRange) { ----------------------" << endl;

  cout<<Plane<<" "<<NewPlane<<" "<<GoForward<<" "<<endl;
  for (int i=0; i<5; ++i) {
    for (int j=0; j<5; ++j) {
      F_k_minus[i][j]=0;  
    }
  ...
  while((SwimInc==false || SwimDec==false) && nswimfail<=10) {
    ...
    if(SwimInc==false || SwimDec==false) {
      ...
      // GMA reopen this      cout <<" InoOldTrackFitAlg : GetCombiPropagator, Swim failed - Double momentum and swim again" << endl;
      x_k_minus[4]*=0.5;
  ...
  if(nswimfail>=10) {cout <<" InoOldTrackFitAlg : GetCombiPropagator, nswimfail>10, fail track" << endl; return false;}
...
bool InoOldTrackFitAlg::Swim(double* StateVector, double* Output, const int Plane, 
			  const int NewPlane,const bool GoForward_loc, double* dS, double* Range, double* dE) {

  if(debug_new)   cout <<" ----------------------InoOldTrackFitAlg : Swim(double* StateVector, double* Output, const int Plane, const int NewPlane,const bool GoForward_loc, double* dS, double* Range, double* dE) { ----------------------"<< Plane<<" "<<NewPlane<<endl;
  double loc_layerthickness = 0.096; // in meter
    loc_layerthickness = LayerThickness;//rsa 0.101
  if(Plane>NewPlane) {
  ...
  halfLayerThickness =0.5* LayerThickness; //RSA
  // Initialisations
  // customize for bfield scaling.
  //  BField * bf = new BField(*vldc,-1,0);
  cout<<"loc_layerthickness:  "<<loc_layerthickness<<"halfLayerThickness:  "<<halfLayerThickness<<endl;
  SwimSwimmer* myswimmer = new SwimSwimmer(loc_layerthickness, halfLayerThickness,NewPlane); //*vldc,bf);
  //if(debug_fit)
  int cplane = (ZIncreasesWithTime)? Plane+1 : Plane;
  myswimmer->SetBPlane(cplane);
  if(debug_new) {
     cout <<" InoOldTrackFitAlg : Swim "<< Plane<<" "<<NewPlane<<endl;
     cout <<" InoOldTrackFitAlg : Swim StateVector "<<StateVector[0]<<" "<<StateVector[1]<<" "<<StateVector[2]<<" "<<StateVector[3]<<" "<<StateVector[4]<<" "<<endl;
  }
...
bool InoOldTrackFitAlg::Swim(double* StateVector, double* Output, const int Plane, 
  ...
  cout<<"InoOldTrackFitAlg::Swim(double* StateVector, double* Output, const int Plane,  const double zend, const bool GoForward_loc, double* dS, double* Range, double* dE) {"<<endl;
...
bool InoOldTrackFitAlg::Swim(double* StateVector, double* Output, const double zzz, 
			  const int NewPlane,const bool GoForward_loc, double* dS, double* Range, double* dE) {
  if(debug_fit) cout <<" ----------------------InoOldTrackFitAlg : Swim, specified starting Z ----------------------" << endl;
...
bool InoOldTrackFitAlg::Swim(double* StateVector, double* Output, const int Plane, 
                          const double Zend,const bool GoForward_loc, double* dS, double* Range, double* dE)
{
  cout <<" InoOldTrackFitAlg : Swim, specified end Z" << endl;
...
void InoOldTrackFitAlg::ResetCovarianceMatrix()
{
...
void InoOldTrackFitAlg::GetInitialCovarianceMatrix(const bool FirstIteration)
{
 
  if (debug_fit)
    cout <<"---------------------- InoOldTrackFitAlg : GetInitialCovarianceMatrix---------------------- " << FirstIteration << endl;
...
void InoOldTrackFitAlg::GetNoiseMatrix(const int Plane, const int NewPlane) {
  if(debug_new) cout<<"InoOldTrackFitAlg::GetNoiseMatrix(const int "<<Plane<<", const int "<<NewPlane<<")"<<endl;
 ...
 //   cout <<" InoOldTrackFitAlg : GetNoiseMatrix" << endl;
...
void InoOldTrackFitAlg::ExtrapCovMatrix() {
  // C_k_intermediate = (F_k_minus * C_k_minus * F_k_minus^T) + Q_k_minus
 if(debug_fit)  cout <<" ----------------------InoOldTrackFitAlg : ExtrapCovMatrix----------------------" << endl;
  ...
  for(int i=0; i<5; ++i) {
    if(C_k_intermediate[i][i]<covlim) {
      //GMA reopen this      cout <<" InoOldTrackFitAlg : Negative diagonal element in C_k_intermediate" << endl;
...
void InoOldTrackFitAlg::CalcKalmanGain(const int NewPlane)
{
  // K_k = C_k_intermediate * H_k^T * ( V_k + H_k * C_k_intermediate * H_k^T )^-1
  //                                            ==A1_k
  //                                    =    A2_k
  //                                 =  B2_k; 
 if(debug_fit)    cout <<" InoOldTrackFitAlg : CalcKalmanGain" << endl;
  ...
  //Inverse matrix;
  //  cout <<" InoOldTrackFitAlg : V_k " << NewPlane<<" "<<TrkClustsData[NewPlane][0].XPosErrSq <<" " <<TrkClustsData[NewPlane][0].YPosErrSq<<" "<<PlaneView<<" "<<determinant<<" "<<A1_k[0][0]<<" "<<A1_k[0][1]<<" "<<A1_k[1][0]<<" "<<A1_k[1][1]<<endl;
  if (determinant !=0) {
    ...
    //    cout <<" InoOldTrackFitAlg : Kalman Gain: "<<NewPlane<<" "
 ...
 //   cout <<" InoOldTrackFitAlg : V_k + (H_k * C_k_intermediate * H_k_transpose) is zero!" << endl;
  }
}



void InoOldTrackFitAlg::UpdateStateVector(const int Plane, const int NewPlane, const bool GoForward)
{  
  // x_k = (F_k_minus * x_k_minus) + K_k(m_k - (H_k * F_k_minus * x_k_minus) )
  
  if(TrkFitterDebug>10) {
    cout <<" ----------------------InoOldTrackFitAlg : UpdateStateVector---------------------- "<<Plane<<" , "<<NewPlane << endl;
  }
  int nswimfail=0;
  ...
  if(TrkFitterDebug>10) {
    cout <<" InoOldTrackFitAlg : state: Plane " << Plane<<" NewPlane "
  ...
  while(GetPrediction==false && nswimfail<=10) {
    ...
    */
    ...
    if(GetPrediction==false) {
      ...
      // 	   <<endl;
      // cout <<" InoOldTrackFitAlg : UpdateStateVector, Prediction failed - Double momentum and swim again" << endl;
    }
  }

  if(nswimfail>10) {  // Swim shouldn't fail, as it succeeded to get the propagator
    //GMA reopen this    cout <<" InoOldTrackFitAlg : UpdateStateVector, nswimfail>10, fail track" << endl;
  ...
  //  cout <<" InoOldTrackFitAlg : UpdateStateVector, Check predicted state " << endl;
  ...
  //  cout <<" InoOldTrackFitAlg : UpdateStateVector, Check filtered state " << endl;
...
void InoOldTrackFitAlg::UpdateCovMatrix()
{
  // C_k = (Identity - (K_k * H_k) ) * C_k_intermediate
  if(debug_fit)
    cout <<"---------------------- InoOldTrackFitAlg : UpdateCovMatrix----------------------" << endl;
  double A1_k[5][5];
  ...
  for(int i=0; i<5; ++i) {
    if(C_k[i][i]<covlim) {
      //GMA reopen this      cout <<" InoOldTrackFitAlg : Negative diagonal element in C_k" << endl;
      C_k[i][i]=covlim;
    }
  }
...
void InoOldTrackFitAlg::MoveArrays()
{
  // Move k to k-1 ready to consider next clust

 if(debug_fit)  cout <<"----------------------InoOldTrackFitAlg : MoveArrays 1----------------------" << endl;
  for (int i=0; i<5; ++i) {
    for (int j=0; j<5; ++j) { 
 ...
 if(debug_fit)  cout <<"----------------------InoOldTrackFitAlg : MoveArrays end 1----------------------" << endl;
}



void InoOldTrackFitAlg::CheckValues(double* Input, const int NewPlane)
{
  ...
  if(fabs(Input[4])>10.0) EndofRange=true; 
   ...
   if(Range>0. && (Maxqpfrac*doubleLa/Range)<Maxqp) {Maxqp=(Maxqpfrac*doubleLa/Range);}
     //cout <<" InoOldTrackFitAlg :  Range " << Range << " Maxqp " << Maxqp << endl;

  if(LastIteration) Maxqp=40;
  if(fabs(Input[4])>Maxqp){
       //cout << " CheckValues: Range check correction " << Input[4] << " " << Maxqp << endl;
    // GMA reopen this    cout <<" InoOldTrackFitAlg : CheckValues, Range check correction" << endl;
  ...
  if(fabs(Input[2])>Maxgradient) {
    //GMA reopen this    cout <<" InoOldTrackFitAlg : CheckValues, Gradient correction, U" << endl;
    Input[2]=(Input[2]>0 ? Maxgradient : -Maxgradient);
  }

  if(fabs(Input[3])>Maxgradient) {
    // GMA reopen this    cout <<" InoOldTrackFitAlg : CheckValues, Gradient correction, V" << endl;
  ...
  if(fabs(Input[0])<5000. && fabs(Input[1])<5000.) {PassTrack=true;}
    ...
    cout<< " Passtrack 1 "<<" ievt "<<pAnalysis->ievt<<endl;//rsa
      PassTrack=false;}              //AAR: This is set false after the change in swimswimmer.
}

void InoOldTrackFitAlg::StoreFilteredData(const int NewPlane)
{
  // Store the data required for matching Kalman output data to strips
  if(debug_new) 
    cout <<"----------------------InoOldTrackFitAlg : StoreFilteredData----------------------"<<NewPlane<<" "<< FilteredData[NewPlane].size() << endl;
...
void InoOldTrackFitAlg::StoreFilteredData_sr(const int NewPlane, double* prediction, bool str)
{
  // Store the data required for matching Kalman output data to strips
  if(debug_fit)  
    cout <<"----------------------InoOldTrackFitAlg : StoreFilteredData_sr----------------------  " <<NewPlane<<" "<<str<<" "<<FilteredData[NewPlane].size()<< endl;
  cout<<"ExtraPolData[NewPlane].size() "<<  ExtraPolData[NewPlane].size() <<endl;
  for (unsigned ij=0; ij<ExtraPolData[NewPlane].size(); ij++) {
    if (ExtraPolData[NewPlane][ij].x_k5==0) {
      ExtraPolData[NewPlane].erase(ExtraPolData[NewPlane].begin()+ij); ij-- ;
    }
  ...
  if(debug_fit)  cout <<"----------------------InoOldTrackFitAlg : StoreFilteredData_sr end----------------------" << endl;

}



void InoOldTrackFitAlg::SetTrackProperties(double* Prediction,double* fCMVD, double *fRoof ) { 

  if(nbfield>0) bave /=nbfield;

  // Carry out the assignment of variables to the new fitted track 
  if(TrkFitterDebug>10) {
    cout <<"----------------------InoOldTrackFitAlg : SetTrackProperties----------------------" <<  nbfield <<" " << bave << endl;
  }  
  ...
  cout<<"Vtx Parameter from InoOldTrackFitAlg "<<fCMVD[kk]<<endl;
  }
  fTrackCand->SetExtPara(fCMVD);
    //RSA roof
  for(int ki=0;ki<6;ki++){
  cout<<"Roof Parameter from InoOldTrackFitAlg "<<fRoof[ki]<<endl;
  }
  fTrackCand->SetRoofPara(fRoof);
  ...
  StraightLineFit(finderhitpos,zlayin,strtslope,strtintercpt,strtchisq,strtnhits);
  ...
  double simplecurv,simpleradii;
  double simplex0[2],simplechi2[3],simplexavg[4];
  int simplenhits;
  simple_track_fit(finderhitpos,simplecurv,simpleradii,simplex0,simplechi2,simplexavg,simplenhits);
...
void InoOldTrackFitAlg::TimingFit() //CandFitTrackCamHandle &cth)
{
  if(debug_fit)
    cout <<"----------------------InoOldTrackFitAlg : TimingFit----------------------" << endl;
  ...
  //  cout <<" fInoOldTrackFitAlg : TimingFit" << endl;
  ...
  //  cout <<" eInoOldTrackFitAlg : TimingFit" << endl;  
...
//	
  ...
  //    cout <<" dInoOldTrackFitAlg : TimingFit" << endl;
  ...
  if(n!=0 && CalculateChi2==true) {
    ...
    //    cout <<" cInoOldTrackFitAlg : TimingFit" << endl;
    // Chi2
...
//
  ...
  //  cout <<" bInoOldTrackFitAlg : TimingFit" << endl;
  if(Sq!=0.) {
...
//	   
  ...
  //cout <<" aInoOldTrackFitAlg : TimingFit" << ChiSqPositive <<" "<<ChiSqNdfPos<<" "<<ChiSqNegative<<" "<<ChiSqNdfNeg<< endl;
  if(debug_fit)  cout <<"----------------------InoOldTrackFitAlg : TimingFit----------------------" << endl;
}


void InoOldTrackFitAlg::SetRangeAnddS( ) //CandFitTrackCamHandle& cth)
{

  // Set range and dS as calculated by the swimmer
//  cout <<" InoOldTrackFitAlg : SetRangeAnddS from swimmer values " << endl;
  ...
  */
...

  {
    ...
    //      cout <<" InoOldTrackFitAlg :  Zend on wrong side of last scint! " << endl;
    //      Zend=Zscint;
    //    }
  ...
  for(int i=EndPlane+Increment; Increment*i<=Increment*VtxPlane; i+=Increment) {
	...
	StateVector[5]=FilteredData[i][0].x_k5;
      ...
      if(done){
        ...
        cout <<" InoOldTrackFitAlg :  swim fail " << endl;
      }
      thisplane=i;
    }
  }
...
/*
void InoOldTrackFitAlg::SetPropertiesFromFinderTrack(CandFitTrackCamHandle &cthx)
{
 ...
 cout <<" InoOldTrackFitAlg : SetPropertiesFromFinderTrack" << endl;
...
void InoOldTrackFitAlg::Trace(const char * /* c */) const
{;
}


void InoOldTrackFitAlg::SetT()
{
...
bool InoOldTrackFitAlg::GetCombiPropagator_new(const int Plane, const int NewPlane, const bool GoForward)
{
  // Combination propagator, essentially same as SR propagator, but
  // generation of matrix reduces calls to swimmer by 80%
  cout <<" InoOldTrackFitAlg : GetCombiPropagator_new" << endl;
...
void InoOldTrackFitAlg::UpdateStateVector_new(const int Plane, const int NewPlane, double* Prediction, const bool GoForward)
{  
  // x_k = (F_k_minus * x_k_minus) + K_k(m_k - (H_k * F_k_minus * x_k_minus) )
   //cout <<" InoOldTrackFitAlg : UpdateStateVector" << endl;
  ...
  while(GetPrediction==false && nswimfail<=10) {
    ...
    */
    ...
    if(GetPrediction==false) {
      StateVector[4]*=0.5; 
      nswimfail++; TotalNSwimFail++;
      //GMA reopen this      cout <<" InoOldTrackFitAlg : UpdateStateVector, Prediction failed - Double momentum and swim again" << endl;
    }
  }

  if(nswimfail>10) {  // Swim shouldn't fail, as it succeeded to get the propagator
    //GMA reopen this    cout <<" InoOldTrackFitAlg : UpdateStateVector, nswimfail>10, fail track" << endl;
    cout<< " Passtrack 2 "<<" ievt "<<pAnalysis->ievt<<endl;//rsa
       PassTrack=false;
  }
  ...
  x_k[5] = 1; //GMA 09/02/2009  for all upgrade
   ...
   //Display
    cout <<" InoOldTrackFitAlg : State: Filtered  " << Plane<<" "
         << NewPlane<<" "
         <<TrkClustsData[NewPlane][0].XPos<<" "
...
bool InoOldTrackFitAlg::Swim_new(double* StateVector, double* Output, const int Plane, 
			  int& NewPlane, const bool GoForward_loc, double* dS, double* Range, double* dE)
{

  if(debug_new) cout<<"Swim_new(double* StateVector, double* Output, const int Plane,   int& NewPlane, const bool GoForward_loc, double* dS, double* Range, double* dE)...."<<" "<<Plane<<" "<<NewPlane<<endl;
  double loc_layerthickness = 0.096; // in meter
    loc_layerthickness = LayerThickness;//rsa 0.101
  if( (ZIncreasesWithTime==true && GoForward_loc==false) || 
  ...
  if(NewPlane==10 || NewPlane==11){loc_layerthickness =ZPosLayer[NewPlane] - ZPosLayer[Plane];}//RSA
  cout<<"loc_layerthickness layerthickness:  "<<loc_layerthickness<<" "<<LayerThickness<<endl;
  
  // Initialisations
  // customize for bfield scaling.
  //  BField * bf = new BField(*vldc,-1,0);
  //  SwimSwimmer* myswimmer = new SwimSwimmer(fabs(LayerThickness*(Plane-NewPlane)), 0.5*LayerThickness); //*vldc,bf);
  SwimSwimmer* myswimmer = new SwimSwimmer(Plane, loc_layerthickness, 0.5*LayerThickness,NewPlane); //*vldc,bf);
  int cplane = (ZIncreasesWithTime)? Plane+1 : Plane;
  myswimmer->SetBPlane(cplane);
  //  if(UseGeoSwimmer) GeoSwimmer::Instance()->Initialize(*vldc);
   if(debug_new) {
  ...
  if(fabs(StateVector[4])>1.e-10) {
    ...
    muon.SetCharge(charge);
        cout <<"charge2 === "<<charge<<" "<<momentum.X()<<" "<<momentum.Y()<<" "<<momentum.Z()<<" "<<position.X()<<" "<<position.Y()<<" "<<position.Z()<<" "<<dxdz<<" "<<dydz<<" "<<dsdz<<" st "<<StateVector[0]<<" "<<StateVector[1]<<" "<<StateVector[2]<<" "<<StateVector[3]<<" "<<StateVector[4]<<" "<<muon.GetMomentum().Z()<<endl;
	...
	Output[5]= StateVector[5];
	zexppos = muon.GetPosition().Z();
	cout<<"Zpos: "<< 	Output[5]<<" "<<      muon.GetPosition().Z()       <<endl;
	...
	cout<<"NewPlane: after =nextplane: "<<nextplane<< " "<<NewPlane<<endl;
	//GMA put this more elegantly
	fTrackCand->fdS[NewPlane] =muon.GetS();
	fTrackCand->fRange[NewPlane] =muon.GetRange();
    ...
    if(NewPlane==11){delz = abs(ZPosLayer[NewPlane] - ZPosLayer[Plane]);}//RSA
    cout<<"delz: "<<delz<<endl;
    Output[0]=StateVector[0] + StateVector[2]*delz;
    Output[1]=StateVector[1] + StateVector[3]*delz;
    Output[2]=StateVector[2];
    ...
    cout <<" Output "<< Output[0]<<" "<<Output[1]<<" "<<Output[2]<<" "<<Output[3]<<" "<<Output[4]<<" "<<1/Output[4]<<"zexppos:  "<<zexppos<<" "<<done<<endl;
...
void InoOldTrackFitAlg::GoForwards_new(bool first) { 
  // Carry out the Kalman fit along the track in the direction of increasing z
  if(TrkFitterDebug>0) {
    cout<< endl;
    cout << "InoOldTrackFitAlg : GoForwards_new, carry out fit in positive z direction --- " <<first << endl;
  }
  ...
  for (unsigned ijk=0; ijk<ntrk-iend; ijk++) {
	...
	//	cout<<"goforpref "<<i<<" "<<Prediction[4]<<endl;
	  ...
	  if (PlaneView%2==0) {H_k[0][0]=1;}
	  if (PlaneView   >0) {H_k[1][1]=1;}
	  //cout <<"InoOldTrackFitAlg.GoForwards : WARNING : PlaneView for clusts is not matching with 0/1/2"<<endl;
	  ...
	  double ds_gpl = 0.0;
	  double drange = 0.0;
	  bool CombiPropagatorOk=GetCombiPropagator_new(i,NewPlane,true);//_new rsa
	    ...
	    //	    cout <<" InoOldTrackFitAlg : GoForwards, Filtered q/p " << x_k[4] << endl;
      ...
      //else {cout <<" InoOldTrackFitAlg : GoForwards, Outside of detector - track failed" <<" "<<ijk<<" "<<i<< endl;}
    }
  ...
  if(TrkFitterDebug>0) {
    cout <<" InoOldTrackFitAlg : GoForwards complete "<<first<<endl;
  }
}

void InoOldTrackFitAlg::GoBackwards_new(bool first) { 
  // Carry out the Kalman fit along the track in the direction of decreasing z
  if(TrkFitterDebug>0) {
    cout<<endl;
    cout <<" InoOldTrackFitAlg : GoBackwards_new, carry out fit in negative z direction --- " <<first<< endl;
  }
  double StateVector[6]; double Prediction[6];
  ...
  int iend = (first) ? 0 : 1;
  cout<<"iend: "<<iend<<"ntrkt: "<<ntrk<<endl;
  for (int ijk=ntrk-1; ijk>=iend; ijk--) {
	...
	cout <<"backfirst "<<int(first)<<" "<<ijk<<" "<<ntrk-1<<endl;
	...
	cout<<"..check.."<<endl;
	  ...
	  // cout <<"InoOldTrackFitAlg.GoBackwards : WARNING : PlaneView for clusts is not matching with 0/1/2"<<endl;
	  // bool direction = false; if (NewPlane>i) direction = true;
	  //	  bool CombiPropagatorOk=GetCombiPropagator(i,NewPlane,direction);
	  double ds_gpl = 0.0;
	  double drange = 0.0;
	  bool CombiPropagatorOk=GetCombiPropagator_new(i,NewPlane,false);//use new ?? 06/2022 changed to new
          if(CombiPropagatorOk ) {
            GetNoiseMatrix(i,NewPlane);
            ExtrapCovMatrix();
            CalcKalmanGain(NewPlane);
	    ...
	    //a	    cout <<" InoOldTrackFitAlg : GoBackwards, Filtered q/p " << x_k[4] << endl;
	    //	    cout <<"xarrayBO "<<i<<" "<<NewPlane<<" "<<x_k_minus[0]<<" "<<x_k_minus[1]<<" "<<x_k_minus[2]<<" "<<x_k_minus[3]<<" "<<x_k_minus[4]<<" "<<x_k_minus[5]<<endl;
          }
          else {cout<<" PassTrack 5" <<" ievt "<<pAnalysis->ievt<< endl;PassTrack=false;}//rsa
        }
      }
      //else {cout <<" InoOldTrackFitAlg : GoBackwards, Outside detector - track failed" << endl;}
    }
    //JAM end of range found
  ...
  if(TrkFitterDebug>0) {
    cout <<" InoOldTrackFitAlg : GoBackwards complete "<<first<<endl;
  }
}

bool InoOldTrackFitAlg::DirectionFromFinderHits( InoTrack *trk) {
//  cout<< "  InoOldTrackFitAlg::DirectionFromFinderHits " <<endl;
...
bool InoOldTrackFitAlg::DirectionFromFitterHits(InoTrackCand *trk, int epln, double& xslope, double& xintercept, double& xexp) {
//  cout<< "  InoOldTrackFitAlg::DirectionFromFinderHits " <<endl;
  unsigned nhits = trk->GetEntries();
  double sxy=0; // y (distance) = c t + shift
  double sx=0; // y = distance
...
int InoOldTrackFitAlg::CheckFCPCUpOrDn(double *aax_k, bool DirExtraPol, int MaxMinPlane, bool GoDir) {
  //bit 1 : LocalPos[1];
  //bit 2 : CheckMat[1]
  //bit 3 : LocalPos[0]
  //bit 4 : CheckMat[0]
  
  if(debug_fcpc) cout<<"int InoOldTrackFitAlg::CheckFCPCUpOrDn(double *"<<aax_k[0]<<","<<aax_k[1]<<", bool "<<DirExtraPol<<", int "<<MaxMinPlane<<", bool "<<GoDir<<") {...."<<endl;
...
void InoOldTrackFitAlg::StraightLineFit(vector<Hep3Vector>& finder, vector<int>& loczlay, double* locslope, double* locintercpt, double* locchi2, int* locnhit) {
  ...
  if (snx >0 && sz2*snx - sz*sz !=0) { 
    ...
    for(int ij=0; ij<size; ij++) {
      ...
      //RSA  pAnalysis->hxpos_ext[loczlay[ij]]->Fill((finder[ij].x()-xexpec)/StripXWidth);
      nused++;
    }
    locchi2[0] = chiSquare;
    locnhit[0] = nused;
    ...
    locchi2[0] = locslope[0] = locintercpt[0] = -1; locnhit[0] = -1;
  }
  ...
  if (sny >0 && sz2*sny - sz*sz !=0) { 
    ...
    for(int ij=0; ij<size; ij++) {
      ...
      //RSA      pAnalysis->hypos_ext[loczlay[ij]]->Fill((finder[ij].y()-yexpec)/StripYWidth);
      nused++;
    }
    locchi2[1] = chiSquare;
    locnhit[1] = nused;
  } else {
    locchi2[1] = locslope[1] = locintercpt[1] = -1; locnhit[1] = -1;
  }
...
bool InoOldTrackFitAlg::LinePlaneInt(double* Line, double* Plane, double* Point){
  cout<<"bool InoOldTrackFitAlg::LinePlaneInt(double* Line, double* Plane, double* Point){"<<endl;
  //	G4double Dist;
  //	G4double a, b;
  bool ok;
...
void InoOldTrackFitAlg::simple_track_fit(vector<Hep3Vector>& finder, double& curve, double& radii, double* x0, double* locchisq, double* xavg, int& locnhits) {
  double xerr2 = pow(StripXWidth/pow(12.,0.5),2);
  double yerr2 = pow(StripYWidth/pow(12.,0.5),2);
  errxy2 = xerr2;
