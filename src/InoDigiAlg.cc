...
InoDigiAlg::InoDigiAlg() {
  cout<<"InoDigiAlg::InoDigiAlg() {"<<endl;
  ...
  ShiftInX = paradef->GetShiftInX(); // + paradef->GetStackShift(0);
  ShiftInY = paradef->GetShiftInY(); // + paradef->GetStackShift(1);
  ShiftInZ = paradef->GetShiftInZ(0); // + paradef->GetStackShift(2);
...
void InoDigiAlg::DigitiseSimData() {
 ...
 for (unsigned ij=0; ij<inocal0hit_pointer->InoCal0Hit_list.size(); ij++) {
   inocal0hit_pointer-> InoCal0Hit_list[ij]->Print();}
  ...
  for (unsigned ij=0; ij<inocal0hit_pointer->InoCal0Hit_list.size(); ij++) {
    ...
    int nInX[MxStrip]={-1}; //, -1, -1}; //RSA
    int nInY[MxStrip]={-1};  //, -1, -1};  //RSA
    ...
    cout<<"checkj"<< detectorconfig->GetCollatedIn()<< endl;
    if(detectorconfig->GetCollatedIn()) {
    ...
    double atime = inocal0hit_pointer->InoCal0Hit_list[ij]->GetTime(); 
      ...
      double CorrTimeSmr = gRandom->Gaus(0,TimeCorrSmr); //RSA
 ...
 nymul = GetRandomXY(gapY,pAnalysis->block_ymulsim[nInLA][int(nInX[0]/4.)][int(nInY[0]/4.)]);
	...
	for(int trglx=0; trglx<ntriglay; trglx++) {
      ...
      //      G4double atimeX = tmpatimeX +  G4RandGauss::shoot(0,TimeUnCorrSmr); //RSA
...
void InoDigiAlg::SaveDigiData() {
  cout<<"savedigidata: "<<inocal0hit_pointer<<endl;
...
//   if(inocal0hit_pointer) {inocal0hit_pointer->InoCal0Hit_list.clear(); delete inocal0hit_pointer; inocal0hit_pointer=0;}
 ...
 // 
 cout<<"savedigidata end: "<<inocal0hit_pointer<<endl;
