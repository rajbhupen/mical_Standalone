...
using namespace std;
//......................................................................
SwimSwimmer::SwimSwimmer(double dist, double halfgap,double NewPlane ) : //const VldContext& vldc) :
  //  fMagField(0),
...
SwimSwimmer::SwimSwimmer(int pln, double dist, double halfgap, double NewPlane) : //const VldContext& vldc) :
  //NewPlane added RSA
  //  fMagField(0),
  fStepMin(1.0e-6), //(*Munits::m),
  fAcc(1.0e-4),
...
double SwimSwimmer::Swim(SwimParticle& particle) { // (SwimCondition& c)
  ...
  for (int ij=0; ij<fNmaxStep; ++ij) {
    ...
    // }
    // cout<<" fStep "<< fStep << " "<<ij<<" "<<distToNextPlane<<endl;
    ...
    //======================================================================
    double dirGeom[3] = {particle.GetDirection().X(),particle.GetDirection().Y(),particle.GetDirection().Z()};
    double posGeom[3] = {100*particle.GetPosition().X(),100*particle.GetPosition().Y(),100*particle.GetPosition().Z()};
    icalGeometry->InitTrack(posGeom, dirGeom);//in cm
   icalGeometry->FindNextBoundary();
    cout<<icalGeometry->GetCurrentVolume()->GetName()<<" "<<    endl;
  ...
  if (strstr(icalGeometry->GetCurrentVolume()->GetName(),"IRLAYElog")) {
    ...
    fStep = min(fStepSize,0.01*abs(icalGeometry->GetStep()+epsilon)*abs(particle.GetDirection().Z())); // /max(0.001,direction[2]); //RSA 0.01 is for cm to m
      cout<<"Iron "<<fStep<<endl;
   }
   else{
     fStep = 0.01*abs(icalGeometry->GetStep()+epsilon)*abs(particle.GetDirection().Z() );
   }
      
   // } else {
      cout<<"..check #Steps.."<<ij<<endl;
   ...
   cout<<distToNextPlane<<" "<<fStepSize<<" "<<fDistance<<endl;
  ...
  cout<<distToNextPlane<<" "<<fStepSize<<" "<<fDistance<<endl;
    ...
    if (distToNextPlane>=11.6 || abs(distToNextPlane-fDistance)<epsilon || distToNextPlane==fDistance)                                                                                            
      {
	cout<<"break,,"<<endl;
	break;           // Outside the detector. Exit for loop                                           
      }
    ...
    if (particle.GetMomentumModulus()!=0.0) {
      ...
      cout<<" distToNextPlane: "<<distToNextPlane<< " fHalfLayerThickness: "<<fHalfLayerThickness<<endl;
      ...
      cout<<"ilay: "<<ilay<< "localZ: "<<localZ<<endl;
      double B[6];
      double Bx,By;
      double buvz[3];
      double pos1[3];
      ...
      cout<<"B[0]: "<<B[0]<<" B[1]: "<<B[1]<<" B[2]: "<<B[2]<<" "<< icalGeometry->GetCurrentVolume()->GetName()<<" "<<density<< endl;
   ...
   bool UseNewEloss = true;

   if(UseNewEloss){
 //For EnergyLoss function:
      double state[5]= {xyz.X(),xyz.Y(),direction[0]/direction[2],direction[1]/direction[2],particle.GetCharge()/particle.GetMomentumModulus()};
      
       
      double elos = GetEnergyLoss(state, fStep, xi, T_max, I, localmat);
    
   cout      <<"elos =============="<<tmpmom<<" "<<elos<<" "<<" "<<localmat<<endl;
...
bool SwimSwimmer::SwimForward(SwimParticle& particle, int & nextp, double& b_ave) {  //, (SwimCondition& c)
  cout<<"bool SwimSwimmer::SwimForward(SwimParticle& particle, int & nextp, double& b_ave) {  "<<endl;
  // Arrow of time, true for dt>0, false for dt<0
  //  fStepData->SetIsForward(true);
  fIsForward = true;
  nbfield=0;
...
bool SwimSwimmer::SwimBackward(SwimParticle& particle, int & nextp, double& b_ave) { // , (SwimCondition& c)
  cout<<"bool SwimSwimmer::SwimBackward(SwimParticle& particle, int & nextp, double& b_ave) { // "<<endl;
  // Arrow of time, true for dt>0, false for dt<0
  //  fStepData->SetIsForward(false);
...
//......................................................................
//---------------Get Enegry Loss copied from Kolahal (Pethu)-----------------------------//

double SwimSwimmer::GetEnergyLoss(double *istate, double dz, double &axi, double &aT_max, double &aI, TGeoMaterial* material) {
...
double SwimSwimmer::Swim(SwimParticle& particle, int& nextplane) {  // (SwimCondition& c)
  ...
  for (int ij=0; ij<fNmaxStep; ++ij) {
   ...
   icalGeometry->FindNextBoundary();
    cout<<"Volume: "<<icalGeometry->GetCurrentVolume()->GetName()<<" Step:  "<<icalGeometry->GetStep()<<endl;
    
   if (strstr(icalGeometry->GetCurrentVolume()->GetName(),"IRLAYElog")) {

      fStep = min(fStepSize,0.01*abs(icalGeometry->GetStep()+epsilon)); // /max(0.001,direction[2]); //RSA 0.01 is for cm to m
      cout<<"Iron "<<fStep<<endl;
   }
   else{
     fStep = 0.01*abs(icalGeometry->GetStep()+epsilon);
   }
      
 
   cout<<"..check #Steps.."<<ij<<" " <<fStep*particle.GetDirection().Z()<<" "<<fDistance<<endl;


 if (fabs(posZ+fStepSize*direction[2]-startposXYZ.z())>fDistance) {
 ...
 //For EnergyLoss function:
   ...
   cout      <<"elos =============="<<tmpmom<<" "<<elos<<" "<<" "<<localmat<<endl;
	...
	tmpmom +=density*distep*elos; // 0.175;
      ...
      //  cout<<"check ij:  "<<ij<<endl;
      cout<<fabs(pos[2]-startposXYZ.z())<<" "<<0.5*fStepSize*dir[2]<<endl;
      cout<<"fDistance: "<<fDistance<<" "<<fStep<<endl;
