...
=======================================
  Proper nomenclature of variable (all word separated by "_" or Uppercase character
Not beginjob
but either beginJob or begin_job
===============================================
Use initial condition or loop at the end, e.g.,
  for (int ij=0; ij<3; ij++) {
    int xy=ij+3;
  } // for (int ij=0; ij<3; ij++);  but do not use "{"

  Do not keep this kind of confusing line //star/
========================================
Aoid using statement "goto .."

Don't use m,cm,s etc variables in standard include file, e.g., G4SystemOfUnits.hh, G4UnitsTable.hh

Do not use arbitray name, give some meaningful name such that after a year your can identify it
and also theres can guess what it means
...
void InoTrackFinder::FormTheHits() {
  ...
  // DatabaseManager *DBmanager = DatabaseManager::AnPointer;
  ...
  if (pstripx) {
    for (unsigned ix=0; ix<pstripx->InoStripX_list.size(); ix++) {
      ...
      // if(((DBmanager->StripInfo[nInDT][nInLA][nInMO][nInCH][0][nInX])&0x03) == 3) {
	...
	for (unsigned jy=0; jy<pstripy->InoStripY_list.size(); jy++) {
	  ...
	  // if(((DBmanager->StripInfo[nnindt][nninla][nninmo][nninch][1][nInY])&0x03) == 3) {
	  ...
	  // double loc_pos_xxx[3];
	  // loc_pos_xxx[0] = tmphit->GetXPos()*100;
	  // loc_pos_xxx[1] = tmphit->GetYPos()*100;
	  // loc_pos_xxx[2] = tmphit->GetZPos()*100;
  ...
  //Sir Added during Zoom meeting to explain how to consider single strip events:
      // for (unsigned ix=0; ix<pstripx->InoStripX_list.size(); ix++) {
  ...
  if(TrkFinderDebug==100) {
	...
	cout<< "InoHits():"
	    ...
	    << " Y_Pos=" <<std::setw(8)<< HitBank[ij][jk]->GetYPos()
	    << " Z_Pos=" <<std::setw(8)<< HitBank[ij][jk]->GetZPos()
	  //  << " chg="   <<std::setw(8)<< HitBank[ij][jk]->GetPulse()
	    << " time="  <<std::setw(8)<< HitBank[ij][jk]->GetTime()
	    << " timex="  <<std::setw(8)<< HitBank[ij][jk]->GetXTimeCorr()
	    << " timey="  <<std::setw(8)<< HitBank[ij][jk]->GetYTimeCorr()
	    << endl;
      }
    }
  } // if(TrkFinderDebug==1)
...
*/  
  ...
  if(TrkFinderDebug==100) {
	...
	cout<< "InoCluster "<<std::setw(3)<<jk
	    ...
	    << " time="  <<std::setw(8)<< ClusterBank[ij][jk]->GetTime()
	    << " timex="  <<std::setw(8)<< ClusterBank[ij][jk]->GetBegTime()
...
void InoTrackFinder::FindAllAssociations() {
  ...
  if (TrkFinderDebug==100) {
	...
	for(unsigned int kl=0; kl<segment->GetNAssocSegBeg(); ++kl) {
	  ...
	  InoCluster* ttclr1 = segtmp->GetCluster(1);
	  InoCluster* ttclr2 = segtmp->GetCluster(2);
	  cout
	    << "(" << ttclr0->GetZPlane() << "," << ttclr0->GetBegXPos() << "->" << ttclr0->GetEndXPos() << ") "
	    << "(" << ttclr1->GetZPlane() << "," << ttclr1->GetBegXPos() << "->" << ttclr1->GetEndXPos() << ") "
	    << "(" << ttclr2->GetZPlane() << "," << ttclr2->GetBegXPos() << "->" << ttclr2->GetEndXPos() << ") "
	    << " (" << ttclr0->GetZPlane() << "," << ttclr0->GetBegYPos() << "->" << ttclr0->GetEndYPos() << ") "
...
void InoTrackFinder::JoinTracks() {
  ...
  if(TrkFinderDebug==100) {
	...
	InoTrackSegment* Seg = ViewSegBank[View][ij];
	if(Seg->GetUID()>2) {
	  for (unsigned int ix=0; ix<Seg->GetEntries(); ix++) {
	    cout<<"ix "<<ij<<" "<<ix<<" "<<Seg->GetCluster(ix)->GetZPlane()<<" "<< Seg->GetCluster(ix)->GetXPos()<<" "<<Seg->GetCluster(ix)->GetYPos()<<endl;
	  }
	...
	if(Seg->GetUID()>2) {
	  ...
	  for(unsigned int ixjx=0; ixjx<Seg->GetEntries(); ixjx++) {
	    InoCluster* tclr0 = Seg->GetCluster(ixjx);
	    cout << "(" << tclr0->GetZPlane() << "," << tclr0->GetBegXPos() << "->" << tclr0->GetEndXPos() << "," << tclr0->GetBegXStrip()  << "->" << tclr0->GetEndXStrip()<< ") (" << tclr0->GetZPlane() << "," << tclr0->GetBegYPos() << "->" << tclr0->GetEndYPos() << "," << tclr0->GetBegYStrip()  << "->" << tclr0->GetEndYStrip()<< ") "<<endl;
	  }
	} else { //GMA 24/02/08
...
//MultiSimAnalysis *pAnalysis = MultiSimAnalysis::AnPointer;
  ...
  if(TrkFinderDebug==100) {
    for(View=0; View<1; ++View) {
      cout<<"-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-q-"<<endl;
      cout << "InoTrackFinder FinalTrackBank: " << View <<" "<<FinalTrackBank[View].size()<< endl;
      for(unsigned int ij=0; ij<FinalTrackBank[View].size(); ++ij) {
	InoTrack* qtrk0 = FinalTrackBank[View][ij];
	cout<<"FinalTrackBank "<<ij << qtrk0->ClustsInTrack.size()
	    << "  " << qtrk0->GetBegZPlane() << "  " << qtrk0->GetEndZPlane() << endl;
	for(unsigned int ixjx=0; ixjx<qtrk0->GetEntries(); ixjx++) {
	  cout << "(" << qtrk0->ClustsInTrack[ixjx]->GetZPlane() << "," << qtrk0->ClustsInTrack[ixjx]->GetBegXPos() << "->" << qtrk0->ClustsInTrack[ixjx]->GetEndXPos() << "," << qtrk0->ClustsInTrack[ixjx]->GetBegXStrip()  << "->" << qtrk0->ClustsInTrack[ixjx]->GetEndXStrip()<< ") (" << qtrk0->ClustsInTrack[ixjx]->GetZPlane() << "," << qtrk0->ClustsInTrack[ixjx]->GetBegYPos() << "->" << qtrk0->ClustsInTrack[ixjx]->GetEndYPos() << "," << qtrk0->ClustsInTrack[ixjx]->GetBegYStrip()  << "->" << qtrk0->ClustsInTrack[ixjx]->GetEndYStrip()<< ") " << qtrk0->ClustsInTrack[ixjx]->GetBegTime()<<","<<qtrk0->ClustsInTrack[ixjx]->GetEndTime()<<endl;
	}
      }
    }
  } // if(TrkFinderDebug==2) {
  ...
  */
  ...
  if(TrkFinderDebug==100){
    ...
    for(int View1x=0; View1x<1; ++View1x) {
      for(unsigned int ij=0; ij<FinalTrackBank[View1x].size(); ++ij) {
	InoTrack* ftrk1 = FinalTrackBank[View1x][ij];
	cout<<"Track Num... "<< ij+1 <<endl;
	for(unsigned int ixjx=0; ixjx<ftrk1->GetEntries(); ixjx++) {
	  cout << "(" << ftrk1->ClustsInTrack[ixjx]->GetZPlane() << "," << ftrk1->ClustsInTrack[ixjx]->GetBegXPos() << "->" << ftrk1->ClustsInTrack[ixjx]->GetEndXPos() << "," << ftrk1->ClustsInTrack[ixjx]->GetBegXStrip()  << "->" << ftrk1->ClustsInTrack[ixjx]->GetEndXStrip()<< ") (" << ftrk1->ClustsInTrack[ixjx]->GetZPlane() << "," << ftrk1->ClustsInTrack[ixjx]->GetBegYPos() << "->" << ftrk1->ClustsInTrack[ixjx]->GetEndYPos() << "," << ftrk1->ClustsInTrack[ixjx]->GetBegYStrip()  << "->" << ftrk1->ClustsInTrack[ixjx]->GetEndYStrip()<< ") "<<endl;
	}
      }
    }
  }
...
void InoTrackFinder::CleanAndFilled() {
  ...
  //   for(unsigned int ijxx11=0; ijxx11<FinalTrackBank[View_new11].size(); ++ijxx11) {
  //     cout<< "\n InoTrackFinder_final tracks new 111"
  // 	  << "  " << View_new11 << " " << ijxx11
  // 	  << "  " << FinalTrackBank[View_new11][ijxx11]->GetBegZPlane() << "  " << FinalTrackBank[View_new11][ijxx11]->GetEndZPlane() << endl;
  //   }
  // }
  // cout<< "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"<<endl;

  // InoTrack* track_tmp = 0;
  // for(int View_new=0; View_new<1; ++View_new) {                       //asm
  //   if(FinalTrackBank[View_new].size()>1) {
  //     for(unsigned int ijxx=0; ijxx<FinalTrackBank[View_new].size(); ++ijxx) {
  // 	for(unsigned int jkyy=0; jkyy<FinalTrackBank[View_new].size(); ++jkyy) {
  // 	  if(ijxx==jkyy) continue;
  // 	  if(FinalTrackBank[View_new][ijxx]->GetBegZPlane()<FinalTrackBank[View_new][jkyy]->GetBegZPlane()) {
  // 	    track_tmp = FinalTrackBank[View_new][ijxx];
  // 	    FinalTrackBank[View_new][ijxx] = FinalTrackBank[View_new][jkyy];
  // 	    FinalTrackBank[View_new][jkyy] = track_tmp;
  // 	  }
  // 	}
  //     }
  //   }
  ...
  for(int View=0; View<1; ++View) {                       //asm
	...
	cout<< "\n InoTrackFinder_final tracks"
	    << "  " << View << " " << ij << " " << TrkTemp->ClustsInTrack.size()
	    << "  " << TrkTemp->GetBegZPlane() << "  " << TrkTemp->GetEndZPlane() << endl;
	for(unsigned hk1=0; hk1<TrkTemp->ClustsInTrack.size(); hk1++) {
	  cout<< "Track Cluster "<<std::setw(3)<<hk1
	      << " pln="    <<std::setw(3)<< TrkTemp->ClustsInTrack[hk1]->GetZPlane()
	      << " beX=" <<std::setw(4)<< TrkTemp->ClustsInTrack[hk1]->GetBegXStrip()
	      << "  -  " <<std::setw(4)<< TrkTemp->ClustsInTrack[hk1]->GetEndXStrip()
	      << " beY=" <<std::setw(4)<< TrkTemp->ClustsInTrack[hk1]->GetBegYStrip()
	      << "  -  " <<std::setw(4)<< TrkTemp->ClustsInTrack[hk1]->GetEndYStrip()
	      << " Xpos="<<std::setw(8)<< TrkTemp->ClustsInTrack[hk1]->GetXPos()
	      << " Ypos="<<std::setw(8)<< TrkTemp->ClustsInTrack[hk1]->GetYPos()
	      // << " chg=" <<std::setw(8)<< TrkTemp->ClustsInTrack[hk1]->GetPulse()
	      << " time="  <<std::setw(8)<< TrkTemp->ClustsInTrack[hk1]->GetTime()
	      << " timex="  <<std::setw(8)<< TrkTemp->ClustsInTrack[hk1]->GetBegTime()
	      << " timey="  <<std::setw(8)<< TrkTemp->ClustsInTrack[hk1]->GetEndTime()
	      << endl;
