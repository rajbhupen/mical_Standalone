...
InoRecoAlg::InoRecoAlg(int isInOut) {
  ...
  // pfitTrack  = new InoTrackCand_Manager();
  //  DetectorType = paradef->GetDetectorType();
  isData = (isInOut==2) ? 1 : 0;
  isTrackFit = (MessengerFile->GetTrackFit()==1) ? 1 : 0;
  ...
  ShiftInZ = paradef->GetINOroomPos(2) +paradef->GetStackPosInRoom(2)+ paradef->GetShiftInZ(0);
...
InoRecoAlg::~InoRecoAlg() {
  cout<<"InoRecoAlg::~InoRecoAlg() { "<<nLayer<<endl;
  cout<<"check1"<<endl;

  // cout<<"pinotrack From inoreco ~: "<<   pinotrack <<endl;
  // for (unsigned int ij=0; ij<pinotrack->InoTrack_list.size(); ij++)	{
  //   if (pinotrack->InoTrack_list[ij]) {
  //     delete pinotrack->InoTrack_list[ij];
  //        pinotrack->InoTrack_list[ij]=0;
  //   }
  // }
  // pinotrack->InoTrack_list.clear(); delete pinotrack;





  cout<<"pfittrack From ~: "<<pfitTrack<<endl;
 if (pfitTrack) {pfitTrack->InoTrackCand_list.clear(); delete pfitTrack; pfitTrack=0;}
    cout<<"check2"<<endl;
  ...
  if(isData){ //RSA
    if(inoTDCx_pointer) {
  	...
  	for(int jk=0; jk<8; jk++) {
  	  inoTDCx_pointer->xtdctiming[ij][jk].clear(); //valgrind RSA
  	}
      }
...
void InoRecoAlg::ReadEvent(int ixt) {
 ...
 cout<<"check23"<<endl;
  ...
  for (unsigned ij=0; ij<inoStripX_pointer->InoStripX_list.size(); ij++) {
    ...
    cout<<"11 "<<ij<<" "<<xpos<<" "<<zpos<<" "<<inoStripX_pointer->InoStripX_list[ij]->GetXYPos() << " " << inoStripX_pointer->InoStripX_list[ij]->GetZPos() << " " << inoStripX_pointer->InoStripX_list[ij]->GetGenPosX() << " "<< inoStripX_pointer->InoStripX_list[ij]->GetGenPosZ()<<endl<<endl;
  ...
  for (unsigned ij=0; ij<inoStripY_pointer->InoStripY_list.size(); ij++) {
    ...
    double xpos = (1/m)*( ShiftInX);
    //	double xpos =  (1/m)*( (nInDT-1)*(2*parino[0]+gapino) - parlay[0]  + (2*nInMO+1)*parmod[0] + ShiftInX);
    double shift2y = (paradef->GetnStack()>1) ? (2*nInCH-1)*parchm[1] : 0; //GMA one/two RPC in a layer
...
void InoRecoAlg::PerformTrackReconstruction() {
  ...
  if(1) {//pAnalysis->sw_trigx>3 && pAnalysis->sw_trigy>3) {
	...
	cout<<"Going to Track Fitter..."<<endl;
      magfield= MessengerFile->GetMag();
      // magfield=1;


      cout<<"magfield: "<<magfield<<endl;
      pfitTrack  = new InoTrackCand_Manager();
      cout<<"New  InoTrackCand_Manager() created "<<   pfitTrack  <<endl;
      InoNewTrackFitAlg newtrackfitter;
      InoOldTrackFitAlg oldtrackfitter;
      InoLinearTrackFitAlg slfit;
      if(magfield==0){
	slfit.RunAlg();
	...
	if(pfitTrack->InoTrackCand_list.size()) {
	  for (unsigned jk=0; jk<pfitTrack->InoTrackCand_list.size() ; jk++) {
	    if (ij <pAnalysis->ntrkmx) {
	      cout<<"saverecodatatorootfiles"<<endl;
  ...
  //  pAnalysis->pEventTree->Fill();//RSA

  cout<<"........performtrackrecontruction ends......"<<endl;
}
...
void InoRecoAlg::SaveRecoDataToRootFiles(unsigned int iTrackNum, InoTrackCand* pfittedTrack) {
  cout<<"void InoRecoAlg::SaveRecoDataToRootFiles(unsigned int iTrackNum, InoTrackCand* pfittedTrack) {"<<endl;
  ...
  // // pAnalysis->poszvx[iTrackNum] = pfittedTrack->GetVtxZ(); //rsa
  ...
  cout<<"Position from extpara"<< pAnalysis->posxvx[iTrackNum]<<" "<< pAnalysis->posyvx[iTrackNum]<<" "<<  pAnalysis->poszvx[iTrackNum]<<endl;
  ...
  cout<<"positions from roof: "<< pAnalysis->posxrf[iTrackNum]<<" "<< pAnalysis->posyrf[iTrackNum]<<" "<<  pAnalysis->poszrf[iTrackNum]<<endl;
  ...
  pAnalysis->phiend[iTrackNum] = atan2(pfittedTrack->GetEndDirCosV(),pfittedTrack->GetEndDirCosU());
					 ...
					 if(epln>0) {
				       ...
				       pAnalysis->xxtxerr[iTrackNum]=pfittedTrack->GetVtxUdUError();
				       pAnalysis->xxtyerr[iTrackNum]=pfittedTrack->GetVtxUdVError();
