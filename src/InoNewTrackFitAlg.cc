// extrapolation to cmvd as well as to roof.
// Zincreases with time = false put by hand
// Doing extrapolation to roof in one step: Hence Diffplane is set 1 and totdist is calculated accordingly RSA
// Not use random numbers inside Multiple Scattering function.
//InitTrack corrected
//dbxdx small:jim.
//dbxdx =0 by hand..
// setep size 1mm in iron rest acc to widths

/*
Use 20 RPC and also two sets of dummy RPC on top and bottom to have easier
option of extrapolation.

phi/momentum resolution is better for upgoing muon
There is muon in oppsite direction for downgoing muon
Shift in theta in opposite direction, most likely due to first iron layer

Straight()
Exact matching of positions
Extrapolation, how much ?

Glo Position -5.94995 -10753.9 -1806.62 glb (8.65128,-10750.6,-1806.62)
Glo Position    24.05 -10753.9 -1907.62 glb (9.30191,-10752.7,-1907.62)    : 24.05 ??????
...
InoNewTrackFitAlg::InoNewTrackFitAlg() {
	  cout<<" InoNewTrackFitAlg::InoNewTrackFitAlg() " <<endl;
  ...
  ptrackCollection = InoTrack_Manager::APointer;
  ...
  cout<<" doubleLa "<<doubleLa<<endl;//12
  for (unsigned ijk=0; ijk<doubleLa; ijk++) {
    ...
    cout<< "ZPosLayer "<< ijk <<" "<< ZPosLayer[ijk]<<endl; //this is in meters
  }
  //
  ZPosLayer[10] = ZPosLayer[9]+0.101;//101mm 56+45mm
  ZPosLayer[11] = (1/m)*(paradef->GetParworld(2)-1);//coordinates on roof
...
void InoNewTrackFitAlg::RunAlg( ) { // (AlgConfig & ac,  CandHandle &ch, CandContext &cx)
	   cout<<"InoNewTrackFitAlg::RunAlg( ){..."<<endl;
	   //  InoTrack_Manager *ptrackCollection = InoTrack_Manager::APointer;
  //  InoTrackCand_Manager *inoTrackCand_pointer = InoTrackCand_Manager::APointer;//was not there before

  //if(debug_fit)cout <<"----------------------InoNewTrackFitAlg::RunAlg( ) "<<ptrackCollection->InoTrack_list.size()<<"----------------------"<<endl;
 LOOP:
  ...
  if (ptrackCollection)	{
    cout<<"ptrackCollection "<<ptrackCollection<<endl;
	...
	cout<<nhits1<<endl;
        ...
        x_k_minus[1]=fTrackCand->GetVtxV();
				cout<<"Starting X Y "<< x_k_minus[0]<<" "<<x_k_minus[1]<<" "<<MinPlane<<" "<<MaxPlane<<endl;
        ...
        //     cout<<"FilteredData "<<FilteredData[jk][0].x_k0<<" "<<FilteredData[jk][0].x_k1<<" "<<FilteredData[jk][0].x_k2<<" "<<FilteredData[jk][0].x_k3<<endl;
        //   }
        // }
        ...
        cout<<" fTrackCand->GetNDOF() "<<fTrackCand->GetNDOF()<<endl;
        if (/*Ndt==1 && */fTrackCand->GetNDOF()>0 && fTrackCand->GetNDOF()<1000) {
          //cout<<"Kolahal"<<endl;
	  cout<<fTrackCand<<endl;
          inoTrackCand_pointer->InoTrackCand_list.push_back(fTrackCand);
	cout <<" fittre "<< inoTrackCand_pointer->InoTrackCand_list.size()<<endl;


        } else {
          fTrackCand=0;
        }
       	cout <<" fittrer "<< inoTrackCand_pointer->InoTrackCand_list.size()<<endl;
...
void InoNewTrackFitAlg::TrackElementMerging(double *Tr1, double TargetZ, double *Tr2) {
...
void InoNewTrackFitAlg::InitialFramework() { //const CandSliceHandle* slice,CandContext &cx)
  if(debug_fit) { cout<<"  InoNewTrackFitAlg::InitialFramework() " <<endl;}
...
// void InoNewTrackFitAlg::ShowerStrips() {
//   //a  cout <<"InoNewTrackFitAlg : ShowerStrips, Look for large vertex shower" << endl;
...
void InoNewTrackFitAlg::RunTheFitter( ) {
  if(debug_fit) { cout<< " InoNewTrackFitAlg::RunTheFitter()" <<PassTrack<<endl;}

  // cout <<"InoNewTrackFitAlg : RunTheFitter, Call methods in the appropriate order" << endl;
  ...
  if(ZIncreasesWithTime==true) {
    ...
    if(ClusterFound==true) { //GMA 210625 Why this
    ...
    if(ClusterFound==true) {
      ...
      if(LastIteration){
			...
			int NPlane=10;//cmvd
			 ds=0;
			 drange=0;
			double xvtx_para[5] = {0.0};


			PredictedStateCov(x_k_minus_copy, MaxPlane, NPlane, GoForward, xvtx_para, 0, &ds, &drange);//cmvd
			tmpmom =(abs(xvtx_para[4])>1.e-12) ? 1/xvtx_para[4] : 0;
			cout <<"extrapol "<<NPlane <<" "<<xvtx_para[0]<<" "<<xvtx_para[1]<<" "<<xvtx_para[2]<<" "<<xvtx_para[3]<<" "<<xvtx_para[4]<<" "<<tmpmom<<" "<<ds<<" "<<drange<<" "<<ZPosLayer[NPlane]<<endl;
			...
			xvtx_parameter[5] =zexppos;// ZPosLayer[NPlane];
				//	xvtx_parameter[5] =ZPosLayer[NPlane];
			}//gdml option
			...
			int BuildPlane =11;
		        ds=0;
			drange=0;
			double xvtx_para_roof[5] = {0.0};
			 G4double G_Point[3]={0};
			PredictedStateCov(x_k_minus_copy, MaxPlane, BuildPlane, GoForward, xvtx_para_roof, 0, &ds, &drange);

			cout <<"extrapol to roof "<<BuildPlane <<" "<<xvtx_para_roof[0]<<" "<<xvtx_para_roof[1]<<" "<<xvtx_para_roof[2]<<" "<<xvtx_para_roof[3]<<" "<<xvtx_para_roof[4]<<" "<<tmpmom<<" "<<ds<<" "<<drange<<" "<<ZPosLayer[BuildPlane]<<" "<<zexppos<<endl;
		...
		double dsdzvxrf = sqrt(1.0 + xvtx_para_roof[2]*xvtx_para_roof[2] + xvtx_para_roof[3]*xvtx_para_roof[3]);
			...
			cout<<" zexppos: "<<zexppos<<endl; //is in mm

			xvtx_para4roof[5] =1.e-3*zexppos; //in m


			//For extrapolation goes outside buiding avoid adding edep to momentum.And just do extrapolation using LinePlane Intersection.

			cout<<""<<xvtx_para4roof[5]<<" "<<ZPosLayer[BuildPlane]<< endl;
			if(abs(xvtx_para4roof[5]-ZPosLayer[BuildPlane])>1.e-04 ){
			  //			if(xvtx_para4roof[5] <ZPosLayer[BuildPlane]){
			  cout<<".........LinePlane Intersection to Roof........."<<endl;
			  G4double Plane[6]={0,0,ZPosLayer[BuildPlane],0,0,1};
			  double PI = acos(-1.0);
  ...
  double PI = 3.14159;
  double theta = 0.00;
  theta = (180/PI) * acos(1/sqrt(1+pow(x_k[2],2)+pow(x_k[3],2))); //GMA 210625 Similarly for phi ?
  ...
  //   //if((ChiSquare/(2*n-5))<10 && (ChiSquare/(2*n-5))>0.01 &&(NumInUView>1 && NumInVView>1))
		...
		if (NumInUView>1 && NumInVView>1)	{
			double inputx[6]={0};
			SetTrackProperties(x_k, xvtx_parameter,xvtx_para4roof); //cth);
		} else {
			PassTrack=false;
		}
...
void InoNewTrackFitAlg::RemoveTrkHitsInShw() {
  // If the 'clean' section of track is large enough, remove the track finding
  // data for planes before the ShowerEntryPlane
  if(debug_fit)  { cout <<"InoNewTrackFitAlg : RemoveTrkHitsInShw, Discard track finding data in shower" << endl;}
  ...
  if(NumTrackHitsLeft>5) {
    ...
    cout <<"InoNewTrackFitAlg : RemoveTrkHitsInShw, not enough hits after removal. Must use all finder data." << endl;
    SwimThroughShower=false;
  }
 ...
 void InoNewTrackFitAlg::ShowerSwim() {
...
//   if(debug_fit) {
//     cout<<" =========================================================="<<endl;
//     cout <<"InoNewTrackFitAlg : ShowerSwim, improved track finding in shower" << endl;
//     cout<<" =========================================================="<<endl;
//   }
//   // Initialisations
//   int Plane; int NewPlane;
...
//           H_k[1][1]=1;
//         }
//         //	  cout <<"InoNewTrackFitAlg.Showerswim : WARNING : PlaneView for hits is not matching with 0/1/2"<<endl;
...
void InoNewTrackFitAlg::GetFitData_new(int& Plane1, int& Plane2) {
  ...
  for (unsigned ijk=0; ijk<doubleLa; ijk++) {
	...
	if(SlcClustData[ij][jk].csh->GetNXStripsInClust()>4) continue;
	if(SlcClustData[ij][jk].csh->GetNYStripsInClust()>4) continue;
	...
	//Adding the position resolutions JIM & SP
  if(LastIteration==true) {
    ...
    if(SlcClustData[ij][jkx].csh->GetNXStripsInClust()==2) {

    }
    if(SlcClustData[ij][jkx].csh->GetNXStripsInClust()==3) {

    }

    if(SlcClustData[ij][jkx].csh->GetNYStripsInClust()==2) {

    }
    if(SlcClustData[ij][jkx].csh->GetNYStripsInClust()==3) {
...
void InoNewTrackFitAlg::GetFitData(int& Plane1, int& Plane2) {
  // Loop over the initial track strip data and create the final data for fitting
  if(debug_fit) { cout <<"InoNewTrackFitAlg : GetFitData "<<"Plane1 "<<Plane1<<" Plane2 "<<Plane2<<endl;}
...
void InoNewTrackFitAlg::FillGapsInTrack() {
  ...
  if (debug_fit) { cout <<"InoNewTrackFitAlg :: FillGapsInTrack" << endl;}
...
bool InoNewTrackFitAlg::Swim(double* StateVector, double* Output, const int Plane,
                          const int NewPlane,const bool GoForward, double* dS, double* Range, double* dE) {

  if(debug_fit) {cout <<" InoNewTrackFitAlg : Swim "<< Plane<<" "<<NewPlane<<endl;}
...
bool InoNewTrackFitAlg::Swim(double* StateVector, double* Output, const double zzz,
			  const int NewPlane,const bool GoForward, double* dS, double* Range, double* dE) {
  if(debug_fit) { cout <<" InoNewTrackFitAlg : Swim, specified starting Z" << endl;}
...
void InoNewTrackFitAlg::ResetCovarianceMatrix() {
...
void InoNewTrackFitAlg::GetInitialCovarianceMatrix(const bool FirstIteration1) {
  if (debug_fit) { cout <<" InoNewTrackFitAlg : GetInitialCovarianceMatrix " << FirstIteration1 << endl;}
...
void InoNewTrackFitAlg::CalcKalmanGain(double *x__minus,const int NewPlane) {
  if(debug_fit) {
    cout <<" InoNewTrackFitAlg : CalcKalmanGain" <<x__minus[0]<<" "<<x__minus[1]<<" "<<x__minus[2]<<" "<<x__minus[3]<<" "<<x__minus[4]<< endl;
  }
  ...
  // Inverse matrix;
  // cout <<" InoNewTrackFitAlg : V_k " << NewPlane<<" "<<TrkClustsData[NewPlane][0].XPosErrSq <<" " <<TrkClustsData[NewPlane][0].YPosErrSq<<" "<<PlaneView<<" "<<determinant<<" "<<A1_k[0][0]<<" "<<A1_k[0][1]<<" "<<A1_k[1][0]<<" "<<A1_k[1][1]<<endl;
  if (determinant !=0) {
    ...
    //   cout <<" InoNewTrackFitAlg : V_k + (H_k * C_k_intermediate * H_k_transpose) is zero!" << endl;
  }
}

void InoNewTrackFitAlg::UpdateCovMatrix(const int NewPlane) {
  // C_k = (Identity - (K_k * H_k) ) * C_k_intermediate
  // if(debug_fit) {cout <<"*****InoNewTrackFitAlg : UpdateCovMatrix*****" << endl;}
  ...
  if(debug_fit) {cout <<" InoNewTrackFitAlg : UpdateCovMatrix ends" << endl;}
}
void InoNewTrackFitAlg::MoveArrays(const int NewPlane, const bool GoForward) {
  ...
  if(debug_fit) {
    cout <<" InoNewTrackFitAlg : MoveArrays end" << endl;
  }
}
...
void InoNewTrackFitAlg::SetTrackProperties(double* x_xk, double* y_yk, double* z_zk) {
  ...
  //  if(debug_fit) {cout <<"InoNewTrackFitAlg: SetTrackProperties"<<endl;}
  // -----------------------------------------------------------------
  if(x_xk[4]>0.) {
  ...
  cout<<"Set VtxPlane: "<<VtxPlane<<endl;
  ...
  fTrackCand->SetTheta(acos(1./dsdz));
	...
	double phi = atan2(x_xk[3],x_xk[2]);
	if (ZIncreasesWithTime==false) {
		double PI = acos(-1.0);
		phi +=PI;
		if (phi > PI) { phi -=2*PI;}
  ...
  fTrackCand->SetPhi(phi); //atan2(x_xk[3],x_xk[2]));
  ...
  cout<<"covariance matric display"<<endl;
  for(int ij=0;ij<5;ij++){
    for(int jk=0;jk<5;jk++){
      cout<<ij<<" "<<jk<<" "<<C_k[ij][jk]<<" ";

    }
    cout<<endl;
  }
  cout<<C_k[0][0]<<" yyerror "<<C_k[1][1]<<" " <<C_k[2][3]<<endl;
  ...
  cout<<"check"<<C_k[0][2]<<"   "<<C_k[1][3]<<endl;
   fTrackCand->SetVtxUdUError(pow(C_k[0][2],0.5));
   fTrackCand->SetVtxUdVError(pow(C_k[0][3],0.5));
  fTrackCand->SetVtxVdVError(pow(C_k[1][3],0.5));
  fTrackCand->SetVtxVdUError(pow(C_k[1][2],0.5));
  fTrackCand->SetVtxdUdVError(pow(C_k[2][3],0.5));
  // -----------------------------------------------------------------
  //+++++++++++++++++++++++++++++++End++++++++++++++++++++++++++++++++

  // Vtx and end direction cosines
	//GMA Updated for only miniICAL veto getector
  ...
  cout<<"Vtx Parameter from InoNewTrackFitAlg "<<y_yk[kk]<<endl;
  }
  fTrackCand->SetExtPara(y_yk);
  //RSA roof
  for(int ki=0;ki<6;ki++){
  cout<<"Roof Parameter from InoNewTrackFitAlg "<<z_zk[ki]<<endl;
  }
  fTrackCand->SetRoofPara(z_zk);
  ...
  double sx2=0;
  cout<<"check nan"<<endl;
  cout<<fTrackCand->GetClusterEntries()<<endl;
...
void InoNewTrackFitAlg::TimingFit() { //( CandFitTrackCamHandle &cth)
  if(debug_fit) { cout <<" InoNewTrackFitAlg : TimingFit" << endl;}
  ...
  // cout <<" fInoNewTrackFitAlg : TimingFit" << endl;
  ...
  // cout <<" eInoNewTrackFitAlg : TimingFit" << endl;
  ...
  // cout <<" dInoNewTrackFitAlg : TimingFit" << endl;
  ...
  if(nc!=0 && CalculateChi2==true) {
    ...
    // cout <<" cInoNewTrackFitAlg : TimingFit" << endl;
    // Chi2
  ...
  // cout <<" bInoNewTrackFitAlg : TimingFit" << endl;
  if(Sq!=0.){
  ...
  // cout <<" aInoNewTrackFitAlg : TimingFit" << ChiSqPositive <<" "<<ChiSqNdfPos<<" "<<ChiSqNegative<<" "<<ChiSqNdfNeg<< endl;
}

/*
  void InoNewTrackFitAlg::SetRangeAnddS( ) //CandFitTrackCamHandle& cth)
  {
  // Set range and dS as calculated by the swimmer
  // cout <<" InoNewTrackFitAlg : SetRangeAnddS from swimmer values " << endl;
  ...
  //      cout <<" InoNewTrackFitAlg :  Zend on wrong side of last scint! " << endl;
  //      Zend=Zscint;
  //    }
  ...
  cout <<" InoNewTrackFitAlg :  swim fail " << endl;
  }
  thisplane=i;
  }
  }
...
/*
  void InoNewTrackFitAlg::SetPropertiesFromFinderTrack(CandFitTrackCamHandle &cthx)
  {
  ...
  cout <<" InoNewTrackFitAlg : SetPropertiesFromFinderTrack" << endl;
...
void InoNewTrackFitAlg::Trace(const char * /* c */) const
{;
}
void InoNewTrackFitAlg::SetT() {
...
void InoNewTrackFitAlg::GoForwards(bool first) {
  ...
  unsigned ntrk = fTrackCand->GetClusterEntries();	// number of hits in a track(let)
	//  cout<<"ntrk "<< ntrk<<" first "<< first<<endl;
  ...
  for (unsigned ijk=0; ijk<ntrk-iend; ijk++) {
    ...
    int ij = fTrackCand->ClustsInTrack[ijk]->GetZPlane();
		//		cout <<"Goforwards "<<int(first) <<" "<<ijk<<" "<< ij<<" "<<fTrackCand->ClustsInTrack[ijk]->GetXPos()<<" "<<fTrackCand->ClustsInTrack[ijk]->GetYPos()<<" "<<fTrackCand->ClustsInTrack[ijk]->GetZPos()<<endl;
...
void InoNewTrackFitAlg::GoBackwards(bool first) {
  ...
  for (int ijk=ntrk-1; ijk>=iend; ijk--) {
    ...
    int ij = fTrackCand->ClustsInTrack[ijk]->GetZPlane();
    if (TrkClustsData[ij].size()>0) {
			//			cout <<"Gobackwards "<<int(first) <<" "<<ijk<<" "<< ij<<" "<<fTrackCand->ClustsInTrack[ijk]->GetXPos()<<" "<<fTrackCand->ClustsInTrack[ijk]->GetYPos()<<" "<<fTrackCand->ClustsInTrack[ijk]->GetZPos()<<" "<<PassTrack<<endl;
      ...
      if (PassTrack) {
        ...
        /*
          ...
          //								cout<<"nloop != 0 here; Preditiocn is fed to StateVector"<<endl;
          for(int kl=0; kl<6; ++kl) {
      ...
      //else {cout <<" InoNewTrackFitAlg : GoBackwards, Outside detector - track failed" << endl;}
    }
...
bool InoNewTrackFitAlg::DirectionFromFinderHitsOldFunc(InoTrack *trk,  double& FinderPathLength, double& FinderDistance) {
...
bool InoNewTrackFitAlg::DirectionFromFinderHits(InoTrack *trk,  double& FinderPathLength, double& FinderDistance) {
	//  cout<< "  InoNewTrackFitAlg::DirectionFromFinderHits " <<endl;
  int nhits = trk->GetEntries();
...
void InoNewTrackFitAlg::GetPropagator(double *istate, double Bx, double By, double dBxdx, double dBydx, double dBxdy, double dBydy, double dz, TGeoMaterial* material) {
  for (int ij=0; ij<5; ++ij) {
    for (int jk=0; jk<5; ++jk) {
      F_k_minus[ij][jk]=0;
    }
  ...
  cout<<"d1fdr1: "<<d1fdr1<<endl;
  cout<<"d1fdr1: "<<Nepsln<<endl;
...
bool InoNewTrackFitAlg::PredictedStateCov(double *StateVector, const int Plane, int &NewPlane, const bool GoForward, double* x__minus, int isHalf, double* dS, double* Range) { //, double* dE) {
	//  cout<<"=============== Entered InoNewTrackFitAlg::PredictedStateCov ==============="<<endl;

  int projPlane= NewPlane;

  int DiffPlane;
  DiffPlane = (NewPlane>0 && (!isHalf)) ? abs(NewPlane - Plane) : 1;
  cout<<"PredictedStateCov :isHalf "<<isHalf<<" NewPlane "<<NewPlane<<" Plane "<<Plane<<" GoForward "<<GoForward<<" DiffPlane "<<DiffPlane<<endl;
  double epsilon=1.e-6;					//1micron shift in extrapolation
    if(NewPlane==11){DiffPlane=1;} //30062022 RSA for roof
  //  double GPos[3]={0.0};
  double UxPos[3]={0.0};
  double UyPos[3]={0.0};
  double UxxPos[3]={0.0};
  ...
  MPos[2]=ZPosLayer[Plane]*1.e3;
	  ...
	  cout<<"Plane number "<<Plane<<" to "<<NewPlane<<" "<<ZPosLayer[Plane]<<" with input P "<<1./max(1.e-6,StateVector[4])<<"     N "<<NIter<<endl;
		cout <<"StateVector";  for(int ij = 0; ij<5; ij++){cout<<" "<<ij<<" "<<StateVector[ij];} cout<<endl;

	//	if(StateVector[ij]==0.0) {cout<<"something wrong"<<"     "<<NIter<<"     "<<GoForward<<"     "<<Plane<<"     "<<NewPlane<<"     "<<MinPlane<<"     "<<MaxPlane<<endl;}
    ...
    if (ij == 1) {
      ...
      for (int jk=0; jk<5; jk++) {
        i1state[jk] = StateVector[jk];	// i1state (here) is the vector just about to enter glass layer
				...
				//        cout<<"i1state "<<jk<<" "<<i1state[jk]<<endl;
	  ...
	  cout<<"C_k_i: "<<C_k_intermediate[jk][kl]<<endl;
	}
      }
    }
 ...
 if(NewPlane==11){totdist =ZPosLayer[NewPlane]-ZPosLayer[Plane]; } //30062022 RSA
    ...
    while(abs(dzgl)<=totdist) {
      if (fabs(dzgl)> totdist - 0.000005 ) {

        break;
      }
			//      cout<<"MaxPlane"<<MaxPlane<<endl;
			//      cout<<"MinPlane"<<MinPlane<<endl;
      if (nHit<15 && abs(abs(MaxPlane-MinPlane)-nHit)<5 && (MaxPlane<140 && MinPlane>0)) {f = 4.0;}

			//      cout<<"i1state[2] "<<i1state[2]<<endl;
			//      cout<<"fabs(i1state[2]) "<<fabs(i1state[2])<<endl;
	...
	//  cout<<"check 2"<<endl;
      ...
      if (fabs(i1state[4])>f) {
				//				cout<<"checkx 3 "<< nHit<<" "<<MaxPlane<<" "<<MinPlane<<" "<<f<<" "<< i1state[4]<<endl;
      ...
      // cout<<"dxdz "<<dxdz<<" dsdz "<<dsdz<<" signp" <<signp<<endl;
      ...
      for (int jk=0; jk<3; jk++) {
        posTrack[jk] = 0.1 * MPos[jk]+epsilon*dirTrack[jk];		//convert mm to cm,( raj Thus postrack is in cm)
      }
	...
	// if(      strstr(icalGeometry->GetCurrentVolume()->GetName(),"AirRoomLogic")          ){
      ...
      //   cout<<"i1state"<<endl;
       	for(int ijk=0;ijk<5;ijk++){
       	    ...
       	    cout<<i1state[ijk]<<" " <<i2state[ijk]<<" "<<x__minus[ijk]<<" ";

       	}
       		cout<<endl;
		//   }
	//.
	////	if((NewPlane!=11 && strstr(icalGeometry->GetCurrentVolume()->GetName(),"AirRoomLogic")) || (NewPlane==11 && posTrack[2]>=100*ZPosLayer[11]-1.e-6)){
        	if(NewPlane==10){//cmvd
	   if(strstr(icalGeometry->GetCurrentVolume()->GetName(),"AirRoomLogic")){
	  ////	  	if( abs(posTrack[0])>=200.3 || posTrack[1]>=-873.5 || posTrack[1]<=-1273.5 || posTrack[2]>=100*ZPosLayer[11]-1.e-6){
		cout<<posTrack[2]<<" "<<100*ZPosLayer[10]-2<<endl;//in cm
		cout<<"return  Vol Name: "<<icalGeometry->GetCurrentVolume()->GetName()<<endl;
	      	for(int ijk=0;ijk<5;ijk++){

	      	  x__minus[ijk]=i2state[ijk];//i2state[ijk]
		}
		  zexppos=10*posTrack[2];// in mm since postrack is in cm
	      	  return true;
	...
	//.
		...
		//For Roof:
	if(NewPlane==11){//cmvd
	   if(strstr(icalGeometry->GetCurrentVolume()->GetName(),"WorldLogic")){
	  ////
		cout<<posTrack[2]<<" "<<100*ZPosLayer[11]-2<<endl;//in cm
		cout<<"return  Vol Name: "<<icalGeometry->GetCurrentVolume()->GetName()<<endl;
	      	for(int ijk=0;ijk<5;ijk++){
	...
	// cout<<"i2state[4]: "<<i2state[4]<<endl;
      ...
      *Range += BetheBloch * density * (1.e2*length) * 1.e-3;
      ...
      double bx = 0.0;
      double by = 0.0;
        gGeoManager->InitTrack(posTrack, dirTrack);//RSA
	...
	cout<<".........................."<<endl;
      */
      ...
      for (int jk = 0; jk < 5; jk++) {
        for (int kl = 0; kl < 5; kl++) {
          C_k_minus[jk][kl]	= C_k_intermediate[jk][kl];
	  //	  cout<<"check:: " <<  C_k_intermediate[jk][kl]<<endl;
  ...
  //cout<<dzFe<<"     "<<dznF<<endl;
	//  cout<<"NewPlane "<<NewPlane<<" ZIncreasesWithTime "<<ZIncreasesWithTime<<" nstep "<<nstep<<" "<<MPos[0]<<" "<<MPos[1]<<" "<<MPos[2]<<" "<<1/x__minus[4]<<endl;
...
void InoNewTrackFitAlg::ExtrapCovMatrix() {
  // C_k_intermediate = (F_k_minus * C_k_minus * F_k_minus^T) + Q_k_minus
	//  if(debug_fit) {cout <<" ----------------------InoNewTrackFitAlg : ExtrapCovMatrix----------------------" << endl;}
  ...
  // GMA reopen this      cout <<" InoNewTrackFitAlg : Negative diagonal element in C_k_intermediate" << endl;
...
void InoNewTrackFitAlg::ExtrapCovMatrixall() {
  // C_k_intermediate = (F_k_minus * C_k_minus * F_k_minus^T) + Q_k_minus
  if(debug_fit) {cout <<" ----------------------InoNewTrackFitAlg : ExtrapCovMatrix----------------------" << endl;}
...
/*
  void InoNewTrackFitAlg::CovarianceMatrixExtrapolation() {
  for (int ij=0; ij<5; ++ij) {
  for (int jk=0; jk<5; ++jk) {
...
*/
double InoNewTrackFitAlg::GetEnergyLoss(double *istate, double dz, double &axi, double &aT_max, double &aI, TGeoMaterial* material) {
  ...
  meanExEnergy["FRPCarbon"]        = 78.0E-6; // same as graphite
    meanExEnergy["Polystyrene"] = 68.7E-6; //plastic scintillator taken from nist.gov 06042022
     meanExEnergy["G4_CONCRETE"] = 135.2E-6;
     //  meanExEnergy["G4_PLASTIC_SC_VINYLTOLUENE"] = 64.7E-6; //plastic scintillator taken from nist.gov 06042022
...
void InoNewTrackFitAlg::GetMultipleScattering(double* i2state,double Bx, double By, double dz,/* double xi,*/ double aT_max, double aI, TGeoMaterial* material) {
  //cout<<"*****MS matrix*****"<<endl;
...
void InoNewTrackFitAlg::KalmanFilterStateVector(double *x__minus, const int Plane, const bool GoForward, double x_kk[6]) {
  ...
  M_MinusX_k[1] = M_k[1]-x_kk[1];
    cout<<"check kalmanfilter: "<<x_kk[0]<<" "<<x_kk[1]<<endl;
  if(fabs(x_kk[0])<24.5 && fabs(x_kk[1])<18.5) { //GMA
    PassTrack=true;
  } else {
    cout<<"================================================="<<endl;
...
/*
   bool InoNewTrackFitAlg::CheckSign(double x, double y, double tx, double ty) {
   bool sign;
   double dz;
...
*/
bool InoNewTrackFitAlg::LinePlaneInt(double* Line, double* Plane, double* Point){
  cout<<"bool InoNewTrackFitAlg::LinePlaneInt(double* Line, double* Plane, double* Point){"<<endl;
  //	G4double Dist;
  //	G4double a, b;
  bool ok;
